...
def generate_random_location_within_cape_town():
    ...
    places_result = gmaps.places_nearby(
        # Cape Town's approximate center
...
def generate_users_and_trips(num_users, min_trips_per_user, max_trips_per_user):
    ...
    for _ in range(num_users):
        ...
        for _ in range(num_trips):
            ...
            # Duration between 5 minutes to 2 hours
            duration = random.randint(5, 120)
            end_time = start_time + timedelta(minutes=duration)
            # Distance between 1 to 50 km
            distance = round(random.uniform(1, 50), 2)
            # Risk score between 0 to 10
            risk_score = round(random.uniform(40, 70), 2)
...
def generate_location_near_crime_area(crime_area, radius_km=1):
    ...
    destination = geopy.distance.distance(
        kilometers=random_distance).destination(base_location, random_bearing)
...
users = generate_users_and_trips(
    num_users, min_trips_per_user, max_trips_per_user)
