...
fake = Faker()
gmaps = googlemaps.Client(key='AIzaSyBoBRKnm25Hz_A0TjmvmNdEEJ48LxFLZS8')  # Replace with your API key
...
def generate_users_and_trips(num_users, min_trips_per_user, max_trips_per_user):
    ...
    for _ in range(num_users):
        ...
        for _ in range(num_trips):
            ...
            risk_score = round(random.uniform(40, 70), 2)  # Risk score between 40 to 70
            
            trip_data = {
                "tripId": trip_id,
                "userId": user_id,  # Associate each trip with the user ID
...
def save_data_to_csv(users, trips):
    users_df = pd.DataFrame(users)
    trips_df = pd.DataFrame(trips)
    
    # Save users and trips to CSV files
...
users, trips = generate_users_and_trips(num_users, min_trips_per_user, max_trips_per_user)

# Save users and trips data to CSV
save_data_to_csv(users, trips)
